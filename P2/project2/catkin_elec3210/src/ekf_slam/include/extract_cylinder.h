...
public:
    ExtractCylinder(ros::NodeHandle &n){
        nh_ = n;
        cylinder_marker_pub = nh_.advertise<visualization_msgs::MarkerArray>("cylinder_marker", 1);
        cylinder_cloud_pub = nh_.advertise<sensor_msgs::PointCloud2>("cylinder_cloud", 1);
        non_plane_cloud_pub = nh_.advertise<sensor_msgs::PointCloud2>("non_plane_cloud", 1);
        colored_cloud_pub = nh_.advertise<sensor_msgs::PointCloud2>("colored_cloud", 1);
        reset();
    }

    Eigen::MatrixX2d extract(const pcl::PointCloud<pcl::PointXYZ>::Ptr& cloudIn, std_msgs::Header header){
        reset();
        cloudHeader = header;
        ...
        std::vector<pcl::PointCloud<pcl::PointXYZ>::Ptr> clusters = clustering(non_ground_cloud);
        ...
        for (int i = 0; i < clusters.size(); ++i) {
            ...
            for (int j = 0; j < cluster->points.size(); ++j) {
                ...
                double angle = std::atan2(cluster->points[j].y - center[1], cluster->points[j].x - center[0]) * 180 / M_PI;
                if (angle > 0) positive_angles.push_back(angle);
                else negative_angles.push_back(angle);
            }
            fitness /= cluster->points.size();
            double angle_diff = 0;
            if (positive_angles.size() > 0 && negative_angles.size() > 0){
                double max_positive_angle = *std::max_element(positive_angles.begin(), positive_angles.end());
                double min_negative_angle = *std::min_element(negative_angles.begin(), negative_angles.end());
                double angle_diff1 = 360 - (max_positive_angle - min_negative_angle);
                double min_positive_angle = *std::min_element(positive_angles.begin(), positive_angles.end());
                double max_negative_angle = *std::max_element(negative_angles.begin(), negative_angles.end());
                ...
                angle_diff = *std::max_element(positive_angles.begin(), positive_angles.end()) -
                             *std::min_element(positive_angles.begin(), positive_angles.end());
            } else if (negative_angles.size() > 0){
                angle_diff = *std::max_element(negative_angles.begin(), negative_angles.end()) -
...
//        publishResult();
    ...
    std::vector<pcl::PointCloud<pcl::PointXYZ>::Ptr> clustering(const pcl::PointCloud<pcl::PointXYZ>::Ptr& cloudIn){
            ...
            for (VoxelKey &neighbor: neighbors) {
                ...
                if (neighbor_voxel.instance_id < 0) {
                    ...
                    *clusters[cur_voxel->instance_id] += *neighbor_voxel.cloud();
                    ...
                    voxel_indices[cur_voxel->instance_id].insert(voxel_indices[cur_voxel->instance_id].end(),
    ...
    pcl::PointCloud<pcl::PointXYZ>::Ptr removePlanes(const pcl::PointCloud<pcl::PointXYZ>::Ptr& cloudIn){
