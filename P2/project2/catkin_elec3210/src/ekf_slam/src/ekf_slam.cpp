...
void EKFSLAM::run() {
	...
	while (ros::ok()) {
		...
		Eigen::Vector2d ut =
			Eigen::Vector2d(odomMsg.second->twist.twist.linear.x, odomMsg.second->twist.twist.angular.z);
...
void EKFSLAM::updateMeasurement() {
	...
	for (int i = 0; i < num_obs; ++i) {
		...
		Eigen::Vector2d z_diff = z.segment<2>(2 * i) - z_hat;
		z_diff(1) = normalizeAngle(z_diff(1));
		_mState.segment<2>(3 + idx * 2) +=
			mCov.block<2, 2>(3 + idx * 2, 3 + idx * 2).inverse() * mCov.block<2, 2>(3 + idx * 2, 3 + idx * 2) * z_diff;
